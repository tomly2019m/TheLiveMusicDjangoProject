class MyUtils {
    ...
    generate_text_shadow(e, _, s) {
        if (s = parseInt(s), e = parseInt(e), 0 === s) return "";
        var l = [];
        for (let t = -s; t <= s; t += Math.ceil(s / 4)) for (let a = -s; a <= s; a += Math.ceil(s / 4)) l.push(t + "px " + a + "px " + e + "px " + _);
        return l.join(",") + ";";
    }
    change_li_css(a) {
    ...
    push_black(a, t) {
        ...
        null == e[t][_[t][0]] && null == e[t][_[t][1]] ? console.log("无内容") : (a.$data["black_" + t + "_list"].push(e[t]), 
    ...
    create_time_list(a) {
        let t = 0;
        var e = [], _ = [], s = document.getElementsByClassName("item"), l = s.length;
        if ("lyric" === a) {
            for (;t < l; ) {
                console.log(t);
                try {
                    var o = [];
                    o[0] = s[t].id, o[1] = s[t + 1].id, e[t] = o, t++;
                } catch (a) {
                    var i = [];
                    i[0] = s[t].id, i[1] = "86400", e[t] = i;
                    break;
                }
            }
            _[0] = s[t].id, _[1] = "86400", e[t] = _, console.log(e);
        }
        return {
            lis_len: l,
            time_list: e
    ...
    show_now_lyric(a, t, e, _) {
        if (a < t && parseFloat(e[a][0]) <= _ && _ <= parseFloat(e[a][1]) + 1) {
            var s;
    ...
    music_play(a, t, e) {
        ...
        this.change(a)), this.htmlAudioElement = new Audio(t), this.htmlAudioElement.play().then(() => {
            ...
            "lyric" === e && (_ = s.show_now_lyric(_, l, o, this.currentTime));
    ...
    music_play1(a, t) {
        ...
        const {
            lis_len: _,
            time_list: s
        } = "lyric" === t && this.create_time_list(t);
        "lyric" === t && (this.n = 3 * (parseInt(a.$data.v_li_margin) + parseInt(a.$data.li_height)), 
        this.change(a)), this.htmlAudioElement.play().then(() => {
            this.is_play = 1, console.log(this), console.log(this.is_play);
        }), this.htmlAudioElement.ontimeupdate = () => {
            "lyric" === t && (e = this.show_now_lyric(e, _, s, this.htmlAudioElement.currentTime));
    ...
    base_control(a, t, e) {
        ...
        var _ = e.who_play === a.$data.who_i_am, s = (console.log("is_play: " + t.is_play), 
        0 === e.who_play ? "music" : "lyric");
        e.play_status && !t.is_play && _ ? t.music_play1(a, s) : !e.play_status && t.is_play && _ && (t.htmlAudioElement.pause(), 
        t.is_play = 0), e.replay && _ && (t.htmlAudioElement.load(), t.music_play1(a, s), 
        e = {
            url: a.$data.url,
            data: 0,
            where: "replay",
    ...
    music_info_list_control(a, t, e) {
        console.log(e);
        var e = e.music_name_list, _ = e[0];
        const s = [];
        e.forEach(a => {
            s.push(a.file_name);
        }), a.$data.music_info = s, void 0 !== _ && void 0 !== _.file_name ? t.is_empty(_.file_name[0]) ? $.get("/next_music", {
            url: a.$data.url,
            where: a.$data.who_i_am ? "lyric" : "music"
        }) : a.$data.now_music_name = _.file_name.join(", ") : (a.$data.music_info = [ [ "暂无歌曲", "无" ] ], 
    ...
    who_play_control(a, t, e) {
        e = e.who_play;
        console.log(e), e ? $("[name='switch']:eq(0)").prop("checked", !1) : $("[name='switch']:eq(0)").prop("checked", !0);
    }
    ...
    get_union_data(t, e, a) {
        ...
        a.forEach(a => {
            a = a(t, e);
            _ = [ ..._, ...a.callback_list ], s = [ ...s, ...a.which_data_list ];
        }), this.get_data(t, e, s, _);
    }
    is_empty(t) {
        try {
            return "" === t.trim();
    ...
    is_has_field(a, t) {
        let e = !1;
        if ("object" == typeof a) for (const _ in a) {
            if (_ === t) {
                e = !0;
                break;
            }
    ...
    do_some_command(t, e, a) {
        ...
        let _;
        var s = e.data.play_, l = e.data.replay, o = e.data.lyric_name, i = e.data.is_running, d = e.data.now_music_url, n = e.data.music_name_list, r = e.data.global_setting, c = e.data.user_set;
        switch (a) {
          case "lyric":
        ...
        console.log(n);
        try {
            _ = JSON.parse(JSON.stringify([ n[0] ]));
        } catch (a) {
            _ = [];
        }
        switch (!_.equals([ void 0 ]) && !_.equals([ [ "", "" ] ]) || n.equals([]) || (_ = [], 
        $.get("/next_music", {
            url: t.$data.url,
            where: a
        })), console.log("vm.now_music_name: " + t.$data.now_music_name), console.log("now_music_name: " + _), 
        s && !this.is_play && ("music" === a ? 0 === t.$data.who_play : 1 === t.$data.who_play) && this.htmlAudioElement.play().then(() => {
            this.is_play = 1;
        }), a) {
          case "music":
        ...
        this.change(t)), s || (this.htmlAudioElement.pause(), this.is_play = 0), 
        t.$data.now_music_name = _.join(","), n.equals([]) ? t.$data.music_info = [ [ "暂无歌曲", "无" ] ] : t.$data.music_info = n;
    }
    prepare_to_music_play(a, t, e) {
        var _ = a.$data.now_music_name;
        if (this.htmlAudioElement.pause(), "" === _) $.get("/next_music", {
            url: a.$data.url,
            where: e
    ...
    async test_lyric(a) {
        ...
        var _ = [ [ 2, 6 ], [ 6, 8 ], [ 8, 11 ], [ 11, 14 ], [ 14, 15 ], [ 15, 20 ], [ 20, 25 ], [ 25, 30 ], [ 30, 35 ], [ 35, 40 ], [ 40, 45 ], [ 45, 50 ], [ 50, 55 ], [ 55, 60 ], [ 60, 65 ], [ 65, 70 ], [ 70, 75 ], [ 75, 80 ], [ 80, 85 ], [ 85, 90 ], [ 90, 95 ], [ 95, 100 ], [ 100, 105 ], [ 105, 110 ], [ 110, 115 ], [ 115, 120 ], [ 120, 125 ], [ 125, 130 ], [ 130, 135 ], [ 135, 140 ], [ 140, 145 ], [ 145, 150 ] ];
        for (let a = 0; a < 155 && (await this.delayTime(1e3), !this.global_time_break_flag); a++) console.log(a), 
        t = this.show_now_lyric(t, _.length, _, e), e++;
    ...
    change_load_info(a, t, e, _ = "") {
        switch (a.$data.load_info_flag = t, e) {
          ...
          case "fail":
            a.$data.load_info_icon = "close", a.$data.load_info = "提交失败 原因: " + _;
            break;
    ...
    get_qq_playlist(_) {
        $.getJSON("get-qq-playlist", {
            url: _.$data.url,
            where_url: _.$data.who_i_am ? "lyric_url" : "music_url"
        }, a => {
            console.log(a);
            var t = a.data.disslist;
            for (let a = 1; a < t.length; a++) {
                var e = t[a];
                console.log("list_name: %s, id: %s", e.diss_name, e.tid), _.$data.playlist_name.push({
                    tid: e.tid,
                    name: e.diss_name,
    ...
    set_idle_playlist(t, a, e) {
        t.$data.idle_playlist = [];
        for (const l of e.user_playlist) {
            let a = "";
            var [ _, s ] = l.file_name;
            switch (l.platform) {
              case "qq":
    ...
    change_who_play(a) {
        var t = $("[name='switch']:eq(0)").prop("checked"), t = (console.log(t), 
        {
            url: a.$data.url,
            data: Number(!t),
            where: "who_play",
    ...
    play_pause(a) {
        a.$data.play_icon_flag = a.$data.play_icon_flag ? 0 : 1;
        a = {
            url: a.$data.url,
            data: a.$data.play_icon_flag,
            where: "play",
            where_url: a.$data.where + "_url"
        };
        $.get("play", a);
    ...
    get_music_info(d) {
        $.getJSON("/get_data", {
            ...
            console.log(a);
            let t, e;
            var _ = a.data.music_name_list, s = a.data.who_play, l = a.data.is_running;
            ...
            d.$data.start_flag = !1), _.equals(d.$data.last_list) || (d.$data.last_list = _, 
            d.$data.music_list = _), !t.equals(d.$data.last)) {
    ...
    on_wss(a, t, e) {
        var _, s, l, o;
        "LIVE_OPEN_PLATFORM_DM" === a.cmd && (_ = e[0], s = e[1], e = e[2], {
            music_name: a,
            artist: l,
            status: o
        } = t.my_filter(_, a), _.$data.start_flag && o && ("music_url" === e ? 0 === _.$data.who_i_am : 1 === _.$data.who_i_am) ? t.send_to_server(a, l, s, e) : console.log("stop\n" + a + ": " + l));
    }
    my_filter(a, t) {
        var e = t.data.uid, _ = t.data.msg, s = t.data.uname;
        t.data.guard_level, t.data.fans_medal_name, t.data.fans_medal_level, t.data.fans_medal_wearing_status;
        let l, o = _.split("#")[0].split(" ");
        t = o.shift();
        let i, d;
