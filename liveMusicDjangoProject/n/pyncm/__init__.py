...
from TestModel.models import UsersData
...
class Session(requests.Session):
    ...
    UA_EAPI = "NeteaseMusic/7.2.24.1597753235(7002024);Dalvik/2.1.0 (Linux; U; Android 11; Pixel 2 XL " \
              "Build/RP1A.200720.009) "
    UA_LINUX_API = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.90 " \
                   "Safari/537.36 "
    CONFIG_EAPI = {
        "appver": "9.9.99",
        "buildver": "9009099",
...
def dump_response(key: str, response: dict, cookie: dict, username: str) -> None:
    """
    将返回和cookie存入数据库

    :param key: 'qq', 'cloud', 'ku_wo'
    :param response: 请求的返回
    :param cookie: 请求的cookie
    :param username: 存进此用户
    :return: None
    """
    temp = {'response': response, 'cookie': cookie}
    key_status = (cookie != {})
    try:
        login_status = json.loads(UsersData.objects.get(username=username).login_status)
        login_status[key] = key_status
        database_sessions = json.loads(UsersData.objects.get(username=username).sessions)
        database_sessions[key] = temp
    except (KeyError, TypeError, json.decoder.JSONDecodeError):
        login_status = {"qq": False, "cloud": False, "ku_wo": False, key: key_status}
        default = {'response': {}, 'cookie': {}}
        database_sessions = {"qq": default, "cloud": default, "ku_wo": default, key: temp}
    UsersData.objects.filter(username=username).update(sessions=json.dumps(database_sessions),
                                                       login_status=json.dumps(login_status))
    ...


def load_response(username: str) -> dict:
    """
    取出用户的cookie等信息

    :param username: 要取的用户
    :return: {'response': response, 'cookie': cookie}
    """
    return json.loads(UsersData.objects.get(username=username).sessions)


def GetCurrentSession() -> Session:
    """Retrieves current active session"""
    # get_global_session_manager_from_database('29418115')
